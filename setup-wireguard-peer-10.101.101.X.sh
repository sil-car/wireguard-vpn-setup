...
if [[ ! $1 =~ ^[0-9]{1,3}$ ]]; then
    echo "Need to pass last IP octet; e.g. \"2\" for nate-XPS (10.101.101.2)."
    if [[ -n $1 ]]; then
        echo "\"$1\" was passed."
    fi
    exit 1
...
PUBLIC_IP4="159.223.209.238"
IP4="10.101.101.${ID}"
IP6_108="fded:f4ce:74ba:f54c::"
IP6="${IP6_108}${ID}"
SN4='32'    # IPv4 subnet
SN6='128'   # IPv6 subnet
CLIENT_IFACE='wgtes0'
...
[Peer]
# tess-afr
PublicKey = HaQh9phuCerOhXP7BLscKQk1G7h908F2vDe0UmV63WM=
EndPoint = ${PUBLIC_IP4}:${client_port}
AllowedIPs = 10.101.101.0/24, fded:f4ce:74ba:f54c::/64
PersistentKeepalive = 25
